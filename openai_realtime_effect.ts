...
// Constants
const AUDIO_URL = "https://stream.radiofrance.fr/franceinfo/franceinfo_hifi.m3u8"
...
class WebSocketError extends Data.TaggedError("WebSocketError")<{ cause: unknown }> { }

class OpenAIWebSocket extends Context.Tag("OpenAIWebSocket")<
  OpenAIWebSocket,
  { send: (msg: string) => Effect.Effect<void>; messages: Stream.Stream<ServerEvent> }
>() { }
  ...
  Effect.gen(function* () {
    ...
    const ws = yield* Effect.acquireRelease(
      Effect.async<WebSocket, WebSocketError>((resume) => {
        ...
        ws.addEventListener("error", (e) => resume(Effect.fail(new WebSocketError({ cause: e }))))
      }),
      (ws) => Effect.sync(() => ws.close())
    )

    ws.addEventListener("message", (e) => {
      try { Queue.unsafeOffer(queue, JSON.parse(e.data as string)) } catch { }
    })
...
const audioStream = Command.make(
  ...
  "-analyzeduration", "0", "-i", AUDIO_URL, "-f", "s16le", "-ar", "24000",
  "-ac", "1", "-flush_packets", "1", "-"
).pipe(Command.stream)
...
const program = Effect.gen(function* () {
  ...
  yield* ws.send(JSON.stringify({
    ...
    session: {
      type: "realtime",
      audio: { input: { format: { type: "audio/pcm", rate: 24000 }, turn_detection: null, noise_reduction: null } },
  ...
  yield* Effect.fork(Stream.runForEach(ws.messages, (msg) =>
    Effect.gen(function* () {
  ...
  yield* audioStream.pipe(
    ...
    Stream.runForEach((chunk) =>
      Effect.gen(function* () {
        yield* ws.send(`{"type":"input_audio_buffer.append","audio":"${chunk.toString("base64")}"}`)
        const acc = yield* Ref.updateAndGet(accumulatedBytes, (n) => n + chunk.length)
        const since = yield* Ref.updateAndGet(bytesSinceLastCommit, (n) => n + chunk.length)
        ...
        if (acc >= TARGET_BYTES) {
          yield* Effect.log(`Requesting response (${(acc / BYTES_PER_SECOND).toFixed(1)}s)`)
          yield* ws.send('{"type":"input_audio_buffer.commit"}')
...
BunRuntime.runMain(program.pipe(Effect.provide(Layer.mergeAll(OpenAIWebSocketLive, BunContext.layer))))
